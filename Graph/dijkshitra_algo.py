...
class Graph:
    ...
    def dijkstra(self):
        ...
        for _ in range(self.nVertices - 1):
            min_vertex = self.get_minVertex(visited, distance)
            visited[min_vertex] = True
            for j in range(self.nVertices):
                if (self.adjacencyMatrix[min_vertex][j] != 0 and not visited[j] and
                        distance[j] > distance[min_vertex] + self.adjacencyMatrix[min_vertex][j]):
                    distance[j] = distance[min_vertex] + self.adjacencyMatrix[min_vertex][j]
